...
class Time extends Resource {
  constructor() {
    super({
      ...
      costs: [],
      buildTimeMs: 1000,
      buildDescriptions: ["Generating Power"],
      unitSymbol: { icon: "s", infront: false },
    });
  }
}
...
let resources: AllResourcesObject = { energy, funds, time };
...
let store = new Store([
  ...
  },
  {
    ...
    costs: [
      { resource: "funds", amount: 0.15 },
      { resource: "energy", amount: 0.2 },
    ],
    level: 0,
    maxLevel: 20,
    dependsOn: [],
    onPurchase: (self: StoreItem) => {
      ...
      startingLevel -= self.level * 0.00001;
      startingLevel = startingLevel < 0.1 ? 0.1 : startingLevel; // Cap
      self.description = `Increases the amount of energy generated by ${startingLevel * 100}%`;

      let changeAmount = getChangeAmount(self.level, startingLevel, funds.generateAmount, true);
      funds.generateAmount = changeAmount;

      self.costs = self.costs.map((cost) => {
        cost.amount = getChangeAmount(self.level, 0.3, cost.amount, true);
        return cost;
      });
    },
  },
